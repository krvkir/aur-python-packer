...
def test_resolve_cascade(mock_clone, mock_find_in_dir, mock_aur, mock_repo):
    ...
    mock_repo.side_effect = lambda x: x if x == "pacman" else None # pacman is in repo
    mock_aur.side_effect = lambda x: {"Depends": ["pacman"], "Version": "1.0"} if x == "aur-pkg" else None
...
@patch('aur_python_packer.clients.AURClient.get_info')
@patch('aur_python_packer.resolver.DependencyResolver._find_in_dir')
def test_resolve_pypi(mock_find_in_dir, mock_aur, mock_repo, mock_pypi_meta, mock_pypi_deps, mock_pypi_verify):
...
@patch('aur_python_packer.clients.AURClient.get_info')
@patch('aur_python_packer.resolver.DependencyResolver._find_in_dir')
def test_resolve_pypi_redirect(mock_find_in_dir, mock_aur, mock_repo, mock_pypi_meta, mock_pypi_deps, mock_pypi_verify):
    mock_find_in_dir.return_value = None
    mock_repo.return_value = None
    mock_aur.return_value = None
...
def test_resolve_provides(mock_find_in_dir, mock_aur, mock_repo_provider):
    ...
    mock_repo_provider.side_effect = lambda x: "python-yaml" if x == "python-pyyaml" else (x if x == "python-yaml" else None)
...
def test_search_priority(mock_find_in_dir, mock_aur, mock_repo_provider):
    ...
    mock_find_in_dir.side_effect = lambda pkg, dir: {"path": "packages/pkg", "depends": []} if "packages" in dir else None
    resolver = DependencyResolver(work_dir="work")
